...
export function Modal({ open, onClose, title, description, children, footer, size = 'sm', dismissOnBackdrop = true, variant = 'default' }: ModalProps) {
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- syncing render/closing state to the `open` prop to keep the modal mounted through its exit animation; intended effect-driven state
