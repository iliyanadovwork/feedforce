...
export function ProgressBar({ value, tone = 'accent', className, label }: { value: number; tone?: 'accent' | 'neutral'; className?: string; label?: string }) {
  ...
  return (
    <div className={cn('h-1.5 w-full rounded-full bg-surface-3 overflow-hidden', className)}
      ...
      <div className={cn('h-full rounded-full transition-[width] duration-[var(--dur-base)]', tone === 'accent' ? 'bg-accent' : 'bg-fg')} style={{ width: `${pct}%` }} />
    </div>
  );
}
...
// `align` anchors the bubble horizontally: 'center' (default) centers on the trigger; 'start'/'end'
// pin the bubble's edge to the trigger's — for triggers near a screen/panel edge, where a centered
// bubble would slide under an overlay (e.g. the timeline bar's leftmost button vs the nav rail).
export function Tooltip({ content, side = 'top', align = 'center', children, className }: {
  content: string; side?: 'top' | 'bottom'; align?: 'center' | 'start' | 'end'; children: ReactNode; className?: string;
}) {
  return (
    <span className={cn('relative inline-flex group/tooltip', className)}>
      ...
      <span
        role="tooltip"
        className={cn(
          'pointer-events-none absolute z-dropdown whitespace-nowrap max-w-[240px]',
