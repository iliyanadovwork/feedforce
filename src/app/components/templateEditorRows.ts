// Row ↔ domain mapping for template/post slides. Framework-neutral (no 'use client') so BOTH the
// client editor hook (useTemplateEditor) and server code (automations Template node) can map a raw
// `template_editor_slides` / `template_editor_post_slides` row to a SlideRow and back. The shapes here
// are the single source of truth for the slide column set.
...
import { defaultCarouselSettings, defaultTagStyle } from './templateEditorTypes';
...
export function rowToSettings(row: Record<string, any>): CarouselSettings {
  ...
  return {
    ...
    freeElements:      row.free_elements       ?? d.freeElements,
    layerOrderIds:     row.layer_order_ids     ?? undefined,
    fadeHidden:        row.fade_hidden         ?? false,
    headlineHidden:    row.headline_hidden     ?? false,
    subHidden:         row.sub_hidden          ?? false,
    canvasTransparent: row.canvas_transparent  ?? false,
    ...
    zoneLogoStyles:    row.zone_logo_styles     ?? undefined,
...
export function slideToRow(slide: SlideRow): Record<string, unknown> {
  ...
  return {
    ...
    free_elements:      s.freeElements ?? [],
    layer_order_ids:    s.layerOrderIds ?? [],
    fade_hidden:        s.fadeHidden ?? false,
    headline_hidden:    s.headlineHidden ?? false,
    sub_hidden:         s.subHidden ?? false,
    canvas_transparent: s.canvasTransparent ?? false,
    ...
    zone_logo_styles:   s.zoneLogoStyles ?? null,
