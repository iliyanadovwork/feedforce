...
// 'conflict' = an optimistic-locked save found the row changed by another tab/session and skipped
// the write (see useTemplateEditor's persistSlide); the next edit deliberately overwrites.
export function AutosaveChip({ state }: { state: SaveState }) {
  ...
  if (state === 'conflict') return <Badge tone="warning" className="select-none">Changed elsewhere — edit again to overwrite</Badge>;
  ...
  return (
    <div className="relative group/save flex">
      ...
      <div className="grid place-items-center size-9 rounded-lg group-hover/save:bg-hover transition-colors duration-[var(--dur-fast)] motion-reduce:transition-none">
        <svg width="34" height="34" viewBox="0 0 64 64" fill="none" role="img" aria-label={label} className="select-none shrink-0 text-accent">
